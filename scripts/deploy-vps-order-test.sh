...
activation_stop_line=$(grep -n '^enforce_ci_activation_stop$' "$SCRIPT_DIR/deploy-vps.sh" | cut -d: -f1)
...
[ "$activation_stop_line" -lt "$endpoint_line" ] && [ "$endpoint_line" -lt "$request_line" ] || {
