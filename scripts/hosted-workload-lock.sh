...
if [ "${HOSTED_WORKLOAD_ALLOW_RESOLVED:-0}" = 1 ]; then
  jq -e '.version == 2 and .validatorVersion == "hosted-contract-v2" and (.state == "resolved" or .state == "verified") and (.snapshotGeneration | type == "string" and length > 0) and (.workloadContentSha256 | test("^[a-f0-9]{64}$")) and (.files | type == "array" and length > 0) and (.workloads | type == "array")' "$LOCK" >/dev/null
else
  jq -e '.version == 2 and .validatorVersion == "hosted-contract-v2" and .state == "verified" and (.snapshotGeneration | type == "string" and length > 0) and (.workloadContentSha256 | test("^[a-f0-9]{64}$")) and (.files | type == "array" and length > 0) and (.workloads | type == "array")' "$LOCK" >/dev/null
fi
