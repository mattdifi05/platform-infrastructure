...
function staticSecurityInfraOnlyCheck() {
  ...
  assertMatch(controlCenterLocalStyles, /(?=[\s\S]*--cc-bg)(?=[\s\S]*--cc-surface)(?=[\s\S]*--cc-surface-raised)(?=[\s\S]*--cc-text)(?=[\s\S]*--cc-muted)(?=[\s\S]*--cc-accent)(?=[\s\S]*--cc-radius)(?=[\s\S]*--cc-shadow)(?=[\s\S]*\.cc-app-shell)(?=[\s\S]*\.cc-sidebar)(?=[\s\S]*\.cc-tabs)/, "Control Center must provide a local --cc-* visual system with app shell, sidebar and tabs.");
  ...
  assertMatch(controlCenterTest, /(?=[\s\S]*\/assets\/control-center\/control-center\.css)(?=[\s\S]*\/control\/ui-package)(?=[\s\S]*@stexor\/control-center-local-ui)(?=[\s\S]*cc-app-shell)(?=[\s\S]*cc-sidebar)(?=[\s\S]*cc-tabs)(?=[\s\S]*AppShell)(?=[\s\S]*Control Center UI)/, "Control Center tests must prove the local CSS, shell, tabs and UI contract are served.");
