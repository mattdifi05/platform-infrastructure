...
function staticSecurityInfraOnlyCheck() {
  ...
  assertMatch(controlCenterTest, /\/assets\/stexor-ui\/src\/styles\.css[\s\S]*\/control\/ui-package[\s\S]*UiShell[\s\S]*PillSidebarNav[\s\S]*Design System/, "Control Center tests must prove the real @stexor/ui CSS and API manifest package are served.");
  ...
  assertMatch(controlCenterServer, /navigationForMode[\s\S]*Workers & Jobs[\s\S]*CI\/CD & GitHub Governance[\s\S]*Logs Advanced[\s\S]*Alerts Advanced[\s\S]*Disaster Recovery[\s\S]*Release Evidence[\s\S]*Security Advanced[\s\S]*Billing \/ Plans/, "Control Center Advanced Mode navigation must cover the requested enterprise sections.");
  ...
  assertMatch(controlCenterTest, /(?=[\s\S]*\/control\/advanced)(?=[\s\S]*\/control\/network)(?=[\s\S]*\/control\/advanced\/network)(?=[\s\S]*enterprise-backend)(?=[\s\S]*enterprise-rate-limit)(?=[\s\S]*Route Test Plan)(?=[\s\S]*\/control\/advanced\/cloudflare)(?=[\s\S]*\/control\/advanced\/release-evidence)(?=[\s\S]*\/control\/advanced\/identity)(?=[\s\S]*\/control\/advanced\/secrets)/, "Control Center tests must cover Advanced Mode API evidence endpoints including Network topology.");
