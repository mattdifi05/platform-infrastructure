...
(async () => {
  ...
  const payload = {
    receiver: "platform-synthetic",
    status: "firing",
    alerts: [{
      status: "firing",
      labels: {
        alertname: "PlatformSyntheticAlertDeliveryTest",
        severity: "info",
...
function staticSecurityInfraOnlyCheck() {
  ...
  assertMatch(controlCenterPackage, /"name":\s+"@stexor\/control-center"[\s\S]*"@stexor\/ui":\s+"file:vendor\/@stexor\/ui"/, "Control Center project must declare the vendored @stexor/ui package as a local dependency.");
  assertMatch(controlCenterUiPackage, /"name":\s+"@stexor\/ui"[\s\S]*"exports"[\s\S]*"\.\/styles\.css"[\s\S]*"\.\/ui\.css"/, "Control Center must vendor the real @stexor/ui package instead of a parallel visual system.");
  assertMatch(controlCenterUiManifest, /(?=[\s\S]*"package":\s+"@stexor\/ui")(?=[\s\S]*"UiShell")(?=[\s\S]*"PillSidebarNav")(?=[\s\S]*"StatusPill")/, "Control Center must vendor the real @stexor/ui API manifest with shell, navigation and status primitives.");
  assertMatch(controlCenterUiStyles, /base-01-foundation\.css[\s\S]*ui-02-controls\.css/, "Vendored @stexor/ui styles.css must expose the real foundation and controls stylesheet imports.");
  assertMatch(controlCenterUiAppStyles, /ui-04-app\.css[\s\S]*ui-03-monochrome-surfaces\.css/, "Vendored @stexor/ui ui.css must expose the real app surface stylesheet imports.");
  assertMatch(controlCenterServer, /(?=[\s\S]*serveStaticAsset[\s\S]*\/assets\/stexor-ui\/)(?=[\s\S]*route\(parts, "control", "ui-package"\))(?=[\s\S]*readControlCenterPackageJson)(?=[\s\S]*readStexorUiPackage)(?=[\s\S]*api-manifest\.json)(?=[\s\S]*file:vendor\/@stexor\/ui)(?=[\s\S]*\/assets\/stexor-ui\/src\/styles\.css)(?=[\s\S]*\/assets\/stexor-ui\/src\/ui\.css)/, "Control Center must serve, expose and link the vendored @stexor/ui package contract.");
  assertMatch(controlCenterServer, /class="shell ui-shell ui-experience sx-control-shell"[\s\S]*ui-shell-header[\s\S]*ui-stage-grid[\s\S]*ui-content-sheet/, "Control Center must render through the Stexor UiShell frame contract.");
  assertMatch(controlCenterServer, /function renderControlSidebar[\s\S]*pill-sidebar-nav[\s\S]*data-pill-sidebar-active-index[\s\S]*function renderSectionTabs[\s\S]*pill-tabs[\s\S]*role="tablist"/, "Control Center must use Stexor PillSidebarNav-style macro navigation and PillTabs-style page sections.");
  ...
  assertMatch(controlCenterServer, /Design System[\s\S]*Manifest components[\s\S]*loaded from @stexor\/ui/, "Control Center Settings UI must display the real Stexor UI package contract.");
  assertMatch(controlCenterTest, /(?=[\s\S]*\/assets\/stexor-ui\/src\/styles\.css)(?=[\s\S]*\/control\/ui-package)(?=[\s\S]*@stexor\/control-center)(?=[\s\S]*file:vendor\/@stexor\/ui)(?=[\s\S]*ui-shell)(?=[\s\S]*pill-sidebar-nav)(?=[\s\S]*pill-tabs)(?=[\s\S]*UiShell)(?=[\s\S]*PillSidebarNav)(?=[\s\S]*Design System)/, "Control Center tests must prove the real @stexor/ui CSS, shell, tabs and API manifest package are served.");
