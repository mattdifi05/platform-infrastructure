...
function staticSecurityInfraOnlyCheck() {
  ...
  assertMatch(controlCenterServer, /serveStaticAsset[\s\S]*\/assets\/stexor-ui\/[\s\S]*\/assets\/stexor-ui\/src\/styles\.css[\s\S]*\/assets\/stexor-ui\/src\/ui\.css/, "Control Center must serve and link the vendored @stexor/ui package styles.");
  assertMatch(controlCenterTest, /\/assets\/stexor-ui\/src\/styles\.css[\s\S]*ui-app-01-shell\.css[\s\S]*\.ui-experience/, "Control Center tests must prove the real @stexor/ui CSS package is served.");
