...
if [ -n "${PLATFORM_V1_LIVE_ENV:-}" ]; then
  # env must sit directly after the (single) sudo: a nested sudo would run
  # env_reset and silently strip the PLATFORM_V1_* variables before the
  # bridge could see them (root cause of brownfield-default staging binds).
