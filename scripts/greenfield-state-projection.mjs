...
export function buildStateProjectionPlan({ manifest, includeRedisState = true }) {
  ...
  collected.push(
    entry({
      entryId: "nats-data",
      kind: "nats-data",
      sourceIdentity: "volume:enterprise_nats_data",
      classification: "PRESERVE_IF_STATEFUL",
      backupMethod: "jetstream stream/state capture while quiesced (measure live; manifest size NOT_MEASURED)",
      restoreMethod: "stage captured state into gf-nats data dir backed by greenfield_nats_data before first start",
      target: { kind: "volume", ref: "greenfield_nats_data" },
      ownership: "gf-nats container user",
      mode: "must be classified stateful/empty-by-design at the live phase before cutover; never silently dropped",
      verificationMethod: "stream-count+message-count fingerprint vs source",
      rollbackRelationship: ROLLBACK_VOLUME_UNTOUCHED,
    }),
    entry({
      entryId: "observe-prometheus-data",
      kind: "writable-layer",
      sourceIdentity: "volume:enterprise_prometheus_data",
      classification: "OBSERVE",
      backupMethod: "none (monitoring time series history)",
      restoreMethod: "not restored into greenfield; series regenerate post-cutover",
      target: { kind: "volume", ref: "greenfield_prometheus_data" },
      ownership: "gf-prometheus container user",
      mode: "excluded from projection by classification: monitoring history is regenerated, not migrated",
      verificationMethod: "documented exclusion (zero-data-loss audit trail)",
      rollbackRelationship: ROLLBACK_VOLUME_UNTOUCHED,
    }),
    entry({
      entryId: "observe-loki-data",
      kind: "writable-layer",
      sourceIdentity: "volume:enterprise_loki_data",
      classification: "OBSERVE",
      backupMethod: "none (log aggregation history)",
      restoreMethod: "not restored into greenfield; logs remain queryable in brownfield rollback window",
      target: { kind: "volume", ref: "greenfield_loki_data" },
      ownership: "gf-loki container user",
      mode: "excluded from projection by classification: log history regenerates",
      verificationMethod: "documented exclusion (zero-data-loss audit trail)",
      rollbackRelationship: ROLLBACK_VOLUME_UNTOUCHED,
    }),
    entry({
      entryId: "observe-grafana-alertmanager-pgadmin",
      kind: "writable-layer",
      sourceIdentity: "volumes:enterprise_grafana_data|enterprise_alertmanager_data|enterprise_pgadmin_data",
      classification: "OBSERVE",
      backupMethod: "none (dashboards/alerts/admin-tool state; dashboards are code-managed in repo overlays)",
      restoreMethod: "greenfield starts fresh; dashboard provisioning comes from repository config",
      target: { kind: "volume", ref: "greenfield_grafana_data" },
      ownership: "per-service container users",
      mode: "excluded from projection by classification: UI state and admin tooling metadata only",
      verificationMethod: "documented exclusion (zero-data-loss audit trail)",
      rollbackRelationship: ROLLBACK_VOLUME_UNTOUCHED,
    }),
    entry({
      entryId: "observe-local-registry-cache",
      kind: "writable-layer",
      sourceIdentity: "volume:enterprise_local_registry_data",
      classification: "OBSERVE",
      backupMethod: "none (pull-through/build cache; all consumed images are digest-pinned upstream or rebuilt from exact-main)",
      restoreMethod: "greenfield registry cache rebuilds via greenfield-workload-builder",
      target: { kind: "volume", ref: "greenfield_local_registry_data" },
      ownership: "registry container user",
      mode: "excluded from projection by classification: rebuildable cache",
      verificationMethod: "documented exclusion (zero-data-loss audit trail)",
      rollbackRelationship: ROLLBACK_VOLUME_UNTOUCHED,
    }),
    entry({
...
export function finalSyncWriterRegistry() {
  const registry = [
    ...
    },
    {
      writerId: "nats-data",
      steps: [
        "quiesce: pause NATS publishers/consumers (bus writers) and freeze jetstream state",
        "capture: jetstream state capture from enterprise_nats_data into staged tree",
        "verify: stream-count+message-count fingerprint vs source",
      ],
    },
    {
