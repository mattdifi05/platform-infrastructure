...
- [ ] SBOM archived.
- [ ] `gh workflow run release-attestation.yml --repo OWNER/REPO --ref main` completed, the artifact `github-sigstore-release-evidence` was downloaded, and `sh ./scripts/release-evidence.sh --requireProvenance --imageManifest .tmp/release-attestation/release-subjects.json --sbom reports/release/github-release-sbom-<run-id>.cdx.json --githubAttestation reports/release/github-sigstore-attestation-<stamp>.json --previousImagesFile ./release/previous-images.json` passed with `status=passed`, validated complete GitHub/Sigstore subjects against every image digest and release commit, linked a validated `reports/rollback/rollback-plan-*.json`, and `reports/release/` plus `reports/rollback/` were archived outside Git.
