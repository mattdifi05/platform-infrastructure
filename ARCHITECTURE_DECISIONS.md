...
| --- | --- | --- | --- |
| ADR-001 | Keycloak remains available to application workloads; Control Center administrator identity is application-owned SimpleWebAuthn with no password fallback. | ACCEPTED | Application auth stays unchanged while Control Center credentials, challenges, throttles and revocable sessions live in its PostgreSQL schema. |
| ADR-015 | Control Center has no OIDC client or identity-provider callback. Browser tokens are never persisted and session cookies are opaque hashes in PostgreSQL. | ACCEPTED | Control Center authentication has one authority and no cross-service login handoff. |
...
| ADR-018 | Service identity, not per-account RLS, is the supported data boundary. Every backend/worker uses a distinct credential with exact grants; multi-tenancy requires a new architecture review. | ACCEPTED | T14 introduces grant/cutover/revoke phases and negative cross-table/bucket tests. T18 moves the Stexor-specific implementation out of platform core without weakening this contract. |
| ADR-019 | Release trust comes only from cryptographic GitHub/Sigstore verification bound to exact signer workflow, commit, ref, issuer and subject digest. Normalized reports and unsigned local SLSA are receipts or diagnostics, never admission proof. | ACCEPTED | T16 installs a checksum-pinned verifier, makes governance comparisons exact and removes dynamic values from SSH remote command lines. Provider apply and release publication remain separately approved actions. |
...
| ADR-004 | Stexor business backend, web and workers belong to the hosted Stexor project, not platform core. | ACCEPTED | T18 candidate replaces platform-specific naming and external build coupling with a verified generic workload contract; live cutover remains separately approved. |
