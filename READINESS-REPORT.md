...
- VPS, Cloudflare, WAF, staging, production and managed-secret runbook coverage.
...
- VPS/Ubuntu host readiness script with JSON/Markdown evidence for Docker, Compose, Git, UFW, fail2ban and SSH/Docker hardening.
...
- Healthcheck coverage now renders local WAF, VPS WAF and backup-scheduler Compose stacks and writes service-by-service reports under `reports/healthchecks/`.
...
- VPS Ubuntu LTS bootstrap executed on the actual host with `reports/vps-bootstrap/vps-bootstrap-apply-*.json`.
- VPS Ubuntu LTS hardening executed on the actual host with `reports/vps-hardening/vps-hardening-apply-*.json`.
- VPS host readiness report generated on the actual VPS host with `reports/vps-host/vps-host-readiness-*.json`.
