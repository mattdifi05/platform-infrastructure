...
TMP=$(mktemp -d "$TMP_PARENT/egress-enforcement-poc.XXXXXX")
TMP=$(CDPATH= cd -- "$TMP" && pwd -P)
TMP_ID=$(file_identity "$TMP")
WRAPPER_SENTINEL=$(mktemp "$TMP/.fg068-wrapper-owner.XXXXXX")
WRAPPER_TOKEN=${WRAPPER_SENTINEL##*.}
printf 'FG068-WRAPPER:%s\n' "$WRAPPER_TOKEN" > "$WRAPPER_SENTINEL"
WRAPPER_SENTINEL_ID=$(file_identity "$WRAPPER_SENTINEL")

cleanup() {
  status_code=$?
  trap - EXIT HUP INT TERM
  expected_owner="FG068-WRAPPER:$WRAPPER_TOKEN"
  actual_owner=
  actual_sentinel_id=
  ...
  case "$WRAPPER_SENTINEL" in
    "$TMP"/.fg068-wrapper-owner.*) sentinel_path_owned=1 ;;
...
SOURCE_HEAD_BEFORE=$(safe_git rev-parse --verify 'HEAD^{commit}')
SOURCE_TREE_BEFORE=$(safe_git rev-parse --verify 'HEAD^{tree}')
...
GUARD_ROOT=$(mktemp -d "$TMP/fg068-guard.XXXXXX")
GUARD_ROOT=$(CDPATH= cd -- "$GUARD_ROOT" && pwd -P)
GUARD_SENTINEL=$(mktemp "$GUARD_ROOT/.fg068-owner.XXXXXX")
GUARD_TOKEN=${GUARD_SENTINEL##*.}
printf 'FG068-OWNER:%s\n' "$GUARD_TOKEN" > "$GUARD_SENTINEL"
mkdir "$GUARD_ROOT/source" "$GUARD_ROOT/lab"
safe_git archive --format=tar --output "$GUARD_ROOT/source.tar" "$REVISION"
...
if env -i PATH="$PATH" HOME="$TMP/node-home" LANG=C LC_ALL=C NODE_OPTIONS= \
  FG068_WRAPPER_TEMP_ROOT="$GUARD_ROOT" \
  FG068_OWNERSHIP_SENTINEL="$GUARD_SENTINEL" \
  FG068_OWNERSHIP_TOKEN="wrong-$GUARD_TOKEN" \
  node "$SCRIPT_DIR/egress-ordering-probe.mjs" \
...
RUN_ROOT=$(mktemp -d "$TMP/fg068-run.XXXXXX")
RUN_ROOT=$(CDPATH= cd -- "$RUN_ROOT" && pwd -P)
RUN_SENTINEL=$(mktemp "$RUN_ROOT/.fg068-owner.XXXXXX")
RUN_TOKEN=${RUN_SENTINEL##*.}
printf 'FG068-OWNER:%s\n' "$RUN_TOKEN" > "$RUN_SENTINEL"
mkdir "$RUN_ROOT/source" "$RUN_ROOT/lab"
safe_git archive --format=tar --output "$RUN_ROOT/source.tar" "$REVISION"
...
env -i PATH="$PATH" HOME="$RUN_ROOT" LANG=C LC_ALL=C NODE_OPTIONS= \
  FG068_WRAPPER_TEMP_ROOT="$RUN_ROOT" \
  FG068_OWNERSHIP_SENTINEL="$RUN_SENTINEL" \
  FG068_OWNERSHIP_TOKEN="$RUN_TOKEN" \
  node "$SCRIPT_DIR/egress-ordering-probe.mjs" \
    "$RUN_ROOT/source" "$RUN_ROOT/lab"

SOURCE_HEAD_AFTER=$(safe_git rev-parse --verify 'HEAD^{commit}')
SOURCE_TREE_AFTER=$(safe_git rev-parse --verify 'HEAD^{tree}')
