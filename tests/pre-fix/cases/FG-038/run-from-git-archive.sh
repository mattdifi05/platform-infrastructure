...
TMP=$(mktemp -d "${TMPDIR:-/tmp}/hosted-pid-namespace-poc.XXXXXX")
TMP=$(CDPATH= cd -- "$TMP" && pwd -P)
WRAPPER_SENTINEL=$(mktemp "$TMP/.fg038-wrapper-owner.XXXXXX")
WRAPPER_TOKEN=${WRAPPER_SENTINEL##*.}
printf 'FG038-WRAPPER:%s\n' "$WRAPPER_TOKEN" > "$WRAPPER_SENTINEL"
WRAPPER_SENTINEL_ID=$(file_identity "$WRAPPER_SENTINEL")

cleanup() {
  status=$?
  trap - EXIT HUP INT TERM
  expected_owner="FG038-WRAPPER:$WRAPPER_TOKEN"
  actual_owner=
  actual_sentinel_id=
  ...
  case "$WRAPPER_SENTINEL" in
    "$TMP"/.fg038-wrapper-owner.*) sentinel_path_owned=1 ;;
...
GUARD_ROOT=$(mktemp -d "$TMP/fg038-guard.XXXXXX")
GUARD_ROOT=$(CDPATH= cd -- "$GUARD_ROOT" && pwd -P)
GUARD_SENTINEL=$(mktemp "$GUARD_ROOT/.fg038-owner.XXXXXX")
GUARD_TOKEN=${GUARD_SENTINEL##*.}
printf 'FG038-OWNER:%s\n' "$GUARD_TOKEN" > "$GUARD_SENTINEL"
...
if FG038_WRAPPER_TEMP_ROOT="$GUARD_ROOT" \
  FG038_OWNERSHIP_SENTINEL="$GUARD_SENTINEL" \
  FG038_OWNERSHIP_TOKEN="wrong-$GUARD_TOKEN" \
  node "$SCRIPT_DIR/hosted-pid-namespace-probe.mjs" \
...
RUN_ROOT=$(mktemp -d "$TMP/fg038-run.XXXXXX")
RUN_ROOT=$(CDPATH= cd -- "$RUN_ROOT" && pwd -P)
RUN_SENTINEL=$(mktemp "$RUN_ROOT/.fg038-owner.XXXXXX")
...
FG038_WRAPPER_TEMP_ROOT="$RUN_ROOT" \
  FG038_OWNERSHIP_SENTINEL="$RUN_SENTINEL" \
  FG038_OWNERSHIP_TOKEN="$RUN_TOKEN" \
  node "$SCRIPT_DIR/hosted-pid-namespace-probe.mjs" \
    "$RUN_ROOT/source"
