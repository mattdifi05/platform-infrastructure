...
TMP=$(mktemp -d "$TMP_BASE/hosted-stop-grace-bound-poc.XXXXXX")
TMP=$(CDPATH= cd -- "$TMP" && pwd -P)
TMP_ID=$(file_identity "$TMP")
case "$TMP" in
  "$TMP_BASE"/hosted-stop-grace-bound-poc.*) ;;
  *)
...
OWNER_FILE="$TMP/.hosted-stop-grace-bound-poc-owner"
...
safe_git -C "$SOURCE_REPO" archive --format=tar --output "$TMP/source.tar" "$REVISION"
...
printf '%s\n' '[+] safety in-memory admission only; no Docker CLI, daemon, container, sleep, network, provider, SSH, secret, token, or live access'
env -i \
  ...
  LC_ALL=C \
  NODE_OPTIONS= \
  STOP_GRACE_POC_TMP_ROOT="$TMP" \
  STOP_GRACE_POC_OWNER_TOKEN="$OWNER_TOKEN" \
  "$NODE_BIN" --max-old-space-size=96 "$SCRIPT_DIR/hosted-stop-grace-bound-probe.mjs" "$SOURCE_REAL"
