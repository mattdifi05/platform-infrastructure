...
TMP=$(mktemp -d "$TMP_PARENT/oci-runtime-admission.XXXXXX")
TMP=$(CDPATH= cd -- "$TMP" && pwd -P)
case "$TMP" in
  "$TMP_PARENT"/oci-runtime-admission.*) ;;
  *)
...
OWNER_SENTINEL="$TMP/.oci-runtime-admission-wrapper-owner"
printf 'oci-runtime-admission:%s\n' "$OWNER_TOKEN" > "$OWNER_SENTINEL"
...
cleanup() {
  ...
  valid=true
  actual_parent_id=
  expected_sentinel="$TMP/.oci-runtime-admission-wrapper-owner"
  case "$TMP" in "$TMP_PARENT"/oci-runtime-admission.*) ;; *) valid=false ;; esac
  ...
  [ "$(cat "$OWNER_SENTINEL" 2>/dev/null || true)" = "oci-runtime-admission:$OWNER_TOKEN" ] || valid=false
...
safe_git archive --format=tar --output "$TMP/source.tar" "$REVISION" -- \
  ...
  scripts/runtime-isolation-policy.test.mjs \
...
printf '%s\n' '[+] safety in-memory source-model validation only; no Docker, daemon inventory, container, runtime, network, SSH, credential, service, or live access'
...
env -i \
  ...
  LC_ALL=C \
  NODE_OPTIONS= \
  node --max-old-space-size=64 "$SCRIPT_DIR/oci-runtime-admission-probe.mjs" \
