...
TMP=$(mktemp -d "$TMP_PARENT/cryptographic-admission-verification-poc.XXXXXX")
TMP=$(CDPATH= cd -- "$TMP" && pwd -P)
TMP_ID=$(file_identity "$TMP")
WRAPPER_SENTINEL=$(mktemp "$TMP/.fg063-wrapper-owner.XXXXXX")
WRAPPER_TOKEN=${WRAPPER_SENTINEL##*.}
printf 'FG063-WRAPPER:%s\n' "$WRAPPER_TOKEN" > "$WRAPPER_SENTINEL"
WRAPPER_SENTINEL_ID=$(file_identity "$WRAPPER_SENTINEL")

cleanup() {
  status=$?
  trap - EXIT HUP INT TERM
  expected_owner="FG063-WRAPPER:$WRAPPER_TOKEN"
  actual_owner=
  actual_sentinel_id=
  if [ -f "$WRAPPER_SENTINEL" ] && [ ! -L "$WRAPPER_SENTINEL" ]; then
    actual_owner=$(sed -n '1p' "$WRAPPER_SENTINEL")
    actual_sentinel_id=$(file_identity "$WRAPPER_SENTINEL" 2>/dev/null || true)
  fi
  case "$WRAPPER_SENTINEL" in
    "$TMP"/.fg063-wrapper-owner.*) sentinel_path_owned=1 ;;
  ...
  actual_parent_id=
  if [ -d "$TMP" ] && [ ! -L "$TMP" ]; then
    temp_real=$(CDPATH= cd -- "$TMP" && pwd -P 2>/dev/null || true)
...
actual_tree=$(safe_git rev-parse --verify "$REVISION^{tree}")
...
GUARD_ROOT=$(mktemp -d "$TMP/fg063-guard.XXXXXX")
GUARD_ROOT=$(CDPATH= cd -- "$GUARD_ROOT" && pwd -P)
GUARD_SENTINEL=$(mktemp "$GUARD_ROOT/.fg063-owner.XXXXXX")
GUARD_TOKEN=${GUARD_SENTINEL##*.}
printf 'FG063-OWNER:%s\n' "$GUARD_TOKEN" > "$GUARD_SENTINEL"
...
if env -i PATH="$PATH" HOME="$TMP/node-home" LANG=C LC_ALL=C NODE_OPTIONS= \
  FG063_WRAPPER_TEMP_ROOT="$GUARD_ROOT" \
  FG063_OWNERSHIP_SENTINEL="$GUARD_SENTINEL" \
  FG063_OWNERSHIP_TOKEN="X$GUARD_TOKEN" \
  node "$SCRIPT_DIR/cryptographic-admission-verification-probe.mjs" \
...
RUN_ROOT=$(mktemp -d "$TMP/fg063-run.XXXXXX")
RUN_ROOT=$(CDPATH= cd -- "$RUN_ROOT" && pwd -P)
RUN_SENTINEL=$(mktemp "$RUN_ROOT/.fg063-owner.XXXXXX")
...
env -i PATH="$PATH" HOME="$TMP/node-home" LANG=C LC_ALL=C NODE_OPTIONS= \
  FG063_WRAPPER_TEMP_ROOT="$RUN_ROOT" \
  FG063_OWNERSHIP_SENTINEL="$RUN_SENTINEL" \
  FG063_OWNERSHIP_TOKEN="$RUN_TOKEN" \
  node "$SCRIPT_DIR/cryptographic-admission-verification-probe.mjs" \
    "$RUN_ROOT/source"
