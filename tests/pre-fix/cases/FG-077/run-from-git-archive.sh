...
TMP=$(mktemp -d "$TMP_PARENT/streaming-backup-hash-poc.XXXXXX")
TMP=$(CDPATH= cd -- "$TMP" && pwd -P)
TMP_ID=$(file_identity "$TMP")
WRAPPER_TOKEN=$(random_token)
WRAPPER_SENTINEL="$TMP/.fg077-wrapper-owner.$WRAPPER_TOKEN"
printf 'FG077-WRAPPER:%s\n' "$WRAPPER_TOKEN" > "$WRAPPER_SENTINEL"
WRAPPER_SENTINEL_ID=$(file_identity "$WRAPPER_SENTINEL")

cleanup() {
  status=$?
  trap - EXIT HUP INT TERM
  expected_owner="FG077-WRAPPER:$WRAPPER_TOKEN"
  actual_parent_id=
  actual_tmp_id=
  actual_owner=
  actual_sentinel_id=
  ...
  case "$WRAPPER_SENTINEL" in
    "$TMP"/.fg077-wrapper-owner.*) sentinel_path_owned=1 ;;
  ...
  case "$TMP" in
    "$TMP_PARENT"/streaming-backup-hash-poc.*) temp_path_owned=1 ;;
    *) temp_path_owned=0 ;;
  esac
...
GUARD_ROOT=$(mktemp -d "$TMP/fg077-guard.XXXXXX")
GUARD_ROOT=$(CDPATH= cd -- "$GUARD_ROOT" && pwd -P)
GUARD_TOKEN=$(random_token)
GUARD_SENTINEL="$GUARD_ROOT/.fg077-owner.$GUARD_TOKEN"
printf 'FG077-OWNER:%s\n' "$GUARD_TOKEN" > "$GUARD_SENTINEL"
...
if env -i PATH="$PATH" HOME="$TMP/node-home" LANG=C LC_ALL=C NODE_OPTIONS= \
  FG077_WRAPPER_TEMP_ROOT="$GUARD_ROOT" \
  FG077_OWNERSHIP_SENTINEL="$GUARD_SENTINEL" \
  FG077_OWNERSHIP_TOKEN="wrong-$GUARD_TOKEN" \
  node "$SCRIPT_DIR/streaming-backup-hash-probe.mjs" \
...
RUN_ROOT=$(mktemp -d "$TMP/fg077-run.XXXXXX")
RUN_ROOT=$(CDPATH= cd -- "$RUN_ROOT" && pwd -P)
RUN_TOKEN=$(random_token)
RUN_SENTINEL="$RUN_ROOT/.fg077-owner.$RUN_TOKEN"
...
env -i PATH="$PATH" HOME="$TMP/node-home" LANG=C LC_ALL=C NODE_OPTIONS= \
  FG077_WRAPPER_TEMP_ROOT="$RUN_ROOT" \
  FG077_OWNERSHIP_SENTINEL="$RUN_SENTINEL" \
  FG077_OWNERSHIP_TOKEN="$RUN_TOKEN" \
  node "$SCRIPT_DIR/streaming-backup-hash-probe.mjs" \
