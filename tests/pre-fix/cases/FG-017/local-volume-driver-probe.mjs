...
const LOGICAL_VOLUME = "example-app_data";
...
const CASES = [
  {
    name: "canonical-local-bind",
    definition: {
      driver: "local",
      driver_opts: { type: "none", o: "bind", device: "/synthetic/host-a" },
    },
  },
  {
    name: "workload-prefixed-name-alias",
    definition: {
      name: "example-app_local_bind_alias",
      driver: "local",
      driver_opts: { type: "none", o: "bind", device: "/synthetic/host-b" },
    },
  },
  {
    name: "default-local-driver",
    definition: {
      driver_opts: { type: "none", o: "bind", device: "/synthetic/host-c" },
    },
  },
  {
    name: "bind-rw-option-list",
    definition: {
      driver: "local",
      driver_opts: { type: "none", o: "bind,rw", device: "/synthetic/host-d" },
    },
  },
  {
    name: "rw-bind-option-order",
    definition: {
      driver: "local",
      driver_opts: { type: "none", o: "rw,bind", device: "/synthetic/host-e" },
    },
  },
  {
    name: "read-only-bind-option",
    definition: {
      driver: "local",
      driver_opts: { type: "none", o: "bind,ro", device: "/synthetic/host-f" },
    },
  },
];
...
if (!process.argv[2] || !fs.statSync(sourceRoot, { throwIfNoEntry: false })?.isDirectory()) {
  throw new Error("usage: local-volume-driver-probe.mjs /path/to/archived/source");
}
...
assert.doesNotMatch(contractSource, /combined\.volumes/);
console.log("[+] verified the archived contract never inspects combined.volumes");
...
const directAdmission = admissionResult({}, {
  type: "bind",
  source: "/synthetic/direct-bind",
  target: "/mnt/probe",
});
const directRuntime = runtimeResult({}, {
  type: "bind",
  source: "/synthetic/direct-bind",
  target: "/mnt/probe",
});
assert.equal(directAdmission.accepted, false, "negative control failed: direct bind passed admission");
assert.equal(directRuntime.accepted, false, "negative control failed: direct bind passed runtime isolation");
console.log("[+] negative control direct bind admission=rejected runtime=failed");
...
assert.equal(baselineRuntime.accepted, true, baselineRuntime.error);
console.log("[+] baseline managed named volume admission=accepted runtime=passed");

let vulnerable = 0;
for (const testCase of CASES) {
  const admission = admissionResult(testCase.definition);
  const runtime = runtimeResult(testCase.definition);
  if (admission.accepted && runtime.accepted) {
    vulnerable += 1;
    console.log(`[VULNERABLE] ${testCase.name} logical=${LOGICAL_VOLUME} admission=accepted runtime=passed`);
    continue;
  }
...
console.log(`[+] summary vulnerable=${vulnerable} total=${CASES.length}`);
console.log("[+] no Docker, Compose, mount, volume, network, or selected host directory was accessed");
assert.equal(vulnerable, CASES.length, "the archived vulnerable revision did not accept every local-driver variant");

function admissionResult(volumeDefinition, mountOverride) {
  ...
  combined.services["project-router"].networks.example_app_ingress = null;
  combined.services["example-app-web"] = hostedService(mountOverride);
  combined.networks.example_app_ingress = { internal: true };
  combined.volumes = { [LOGICAL_VOLUME]: structuredClone(volumeDefinition) };
  const lock = {
    workloads: [{
      id: "example-app",
      secrets: [],
...
function runtimeResult(volumeDefinition, mountOverride) {
  const config = runtimeFixture();
  config.services["example-app-web"].volumes = [mountOverride ?? namedMount()];
  config.volumes = { [LOGICAL_VOLUME]: structuredClone(volumeDefinition) };
...
function hostedService(mountOverride) {
  return {
    image: `registry.example/workloads/probe@sha256:${"b".repeat(64)}`,
    read_only: true,
    init: true,
    ...
    volumes: [mountOverride ?? namedMount()],
  };
}
