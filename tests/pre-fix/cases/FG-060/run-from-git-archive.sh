...
TMP=$(mktemp -d "$TMP_PARENT/trusted-forwarded-client-rate-key-poc.XXXXXX")
TMP=$(CDPATH= cd -- "$TMP" && pwd -P)
TMP_ID=$(file_identity "$TMP")
WRAPPER_SENTINEL=$(mktemp "$TMP/.fg060-wrapper-owner.XXXXXX")
WRAPPER_TOKEN=${WRAPPER_SENTINEL##*.}
printf 'FG060-WRAPPER:%s\n' "$WRAPPER_TOKEN" > "$WRAPPER_SENTINEL"
WRAPPER_SENTINEL_ID=$(file_identity "$WRAPPER_SENTINEL")

cleanup() {
  rc=$?
  trap - EXIT HUP INT TERM
  expected_owner="FG060-WRAPPER:$WRAPPER_TOKEN"
  actual_owner=
  actual_sentinel_id=
  physical_tmp=
  actual_tmp_id=
  ...
  case "$WRAPPER_SENTINEL" in
    "$TMP"/.fg060-wrapper-owner.*) sentinel_path_owned=1 ;;
    *) sentinel_path_owned=0 ;;
  esac
  case "${TMP##*/}" in
    trusted-forwarded-client-rate-key-poc.*) temp_name_owned=1 ;;
    *) temp_name_owned=0 ;;
  esac
  if [ -d "$TMP" ] && [ ! -L "$TMP" ] \
    && [ "$physical_tmp" = "$TMP" ] \
...
GUARD_ROOT=$(mktemp -d "$TMP/fg060-guard.XXXXXX")
GUARD_ROOT=$(CDPATH= cd -- "$GUARD_ROOT" && pwd -P)
GUARD_SENTINEL=$(mktemp "$GUARD_ROOT/.fg060-owner.XXXXXX")
GUARD_TOKEN=${GUARD_SENTINEL##*.}
printf 'FG060-OWNER:%s\n' "$GUARD_TOKEN" > "$GUARD_SENTINEL"
archive_revision "$GUARD_ROOT"
GUARD_BEFORE=$(git hash-object "$GUARD_ROOT/source/traefik/dynamic/middlewares.yml")
GUARD_LOG="$GUARD_ROOT/negative-ownership.log"
if env -i PATH="$PATH" HOME="$GUARD_ROOT" LANG=C LC_ALL=C NODE_OPTIONS= \
  REPORT_FG060_WRAPPER_TEMP_ROOT="$GUARD_ROOT" \
  REPORT_FG060_OWNERSHIP_SENTINEL="$GUARD_SENTINEL" \
  REPORT_FG060_OWNERSHIP_TOKEN="wrong-$GUARD_TOKEN" \
  node "$SCRIPT_DIR/trusted-forwarded-client-rate-key-probe.mjs" \
...
GUARD_AFTER=$(git hash-object "$GUARD_ROOT/source/traefik/dynamic/middlewares.yml")
[ "$GUARD_BEFORE" = "$GUARD_AFTER" ] || {
  printf '%s\n' 'guard regression modified archived source' >&2
  exit 1
}

RUN_ROOT=$(mktemp -d "$TMP/fg060-run.XXXXXX")
RUN_ROOT=$(CDPATH= cd -- "$RUN_ROOT" && pwd -P)
RUN_SENTINEL=$(mktemp "$RUN_ROOT/.fg060-owner.XXXXXX")
...
env -i PATH="$PATH" HOME="$RUN_ROOT" LANG=C LC_ALL=C NODE_OPTIONS= \
  REPORT_FG060_WRAPPER_TEMP_ROOT="$RUN_ROOT" \
  REPORT_FG060_OWNERSHIP_SENTINEL="$RUN_SENTINEL" \
  REPORT_FG060_OWNERSHIP_TOKEN="$RUN_TOKEN" \
  node "$SCRIPT_DIR/trusted-forwarded-client-rate-key-probe.mjs" \
    "$RUN_ROOT/source"
