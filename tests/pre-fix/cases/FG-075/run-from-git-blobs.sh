...
[ -d "$TMP_PARENT" ] && [ ! -L "$TMP_PARENT" ] || {
  printf '%s\n' 'temporary parent must be a real directory' >&2
  exit 1
}
TMP_PARENT_ID=$(file_identity "$TMP_PARENT")
TMP=$(mktemp -d "$TMP_PARENT/protected-network-set-poc.XXXXXX")
...
OWNER_SENTINEL="$TMP/.network-set-owner-$OWNER_TOKEN"
...
cleanup() {
  ...
  expected_sentinel="$TMP/.network-set-owner-$OWNER_TOKEN"
  temp_path_valid=false
  case "$TMP" in
    "$TMP_PARENT"/protected-network-set-poc.*) temp_path_valid=true ;;
  esac
  current_owner_id=""
  ...
  if [ "$temp_path_valid" = true ] \
    ...
    && [ "$(cat "$OWNER_SENTINEL")" = "network-set:$OWNER_TOKEN" ]; then
    if rm -rf -- "$TMP"; then
      printf '%s\n' '[+] cleanup wrapper_owned_temp_removed=true sentinel_device_inode_verified=true'
    else
      printf '%s\n' "warning: failed to remove wrapper-owned root: $TMP" >&2
      rc=1
    fi
  else
...
safe_git show "${REVISION}:scripts/hosted-workload-contract.mjs" > "$TMP/source/scripts/hosted-workload-contract.mjs"
safe_git show "${REVISION}:scripts/prepare-hosted-workloads.sh" > "$TMP/source/scripts/prepare-hosted-workloads.sh"
safe_git show "${REVISION}:scripts/compose-vps.sh" > "$TMP/source/scripts/compose-vps.sh"
safe_git show "${REVISION}:compose.networks.yaml" > "$TMP/source/compose.networks.yaml"

PREEXISTING_ROOT="$TMP/preexisting"
mkdir "$PREEXISTING_ROOT"
printf '%s\n' 'preexisting-network-set-control-must-survive' > "$PREEXISTING_ROOT/control.txt"
...
printf '[+] materialized revision=%s tree=%s blobs=4\n' "$REVISION" "$TREE"
...
env -i PATH="$PATH" HOME="$TMP/node-home" LANG=C LC_ALL=C NODE_OPTIONS= \
  node --max-old-space-size=96 "$SCRIPT_DIR/protected-network-set-probe.mjs" \
...
if find "$PREEXISTING_ROOT" -mindepth 1 -maxdepth 1 -name '.network-set-lab-owner-*' -print | grep -q .; then
