...
TMP=$(mktemp -d "$TMP_PARENT/symlink-containment.XXXXXX")
TMP=$(CDPATH= cd -- "$TMP" && pwd -P)
case "$TMP" in
  "$TMP_PARENT"/symlink-containment.*) ;;
  *)
...
OWNER_SENTINEL="$TMP/.symlink-containment-wrapper-owner"
printf 'symlink-containment:%s\n' "$OWNER_TOKEN" > "$OWNER_SENTINEL"
...
cleanup() {
  ...
  valid=true
  expected_sentinel="$TMP/.symlink-containment-wrapper-owner"
  physical_tmp=
  actual_parent_id=
  case "$TMP" in "$TMP_PARENT"/symlink-containment.*) ;; *) valid=false ;; esac
  ...
  [ "$(cat "$OWNER_SENTINEL" 2>/dev/null || true)" = "symlink-containment:$OWNER_TOKEN" ] || valid=false
...
safe_git archive --format=tar --output "$TMP/source.tar" "$REVISION" -- \
  ...
  scripts/hosted-workload-lock.sh \
...
printf '%s\n' '[+] safety synthetic local files and symlinks only; no Docker, network, SSH, credential, service, or live access'
...
env -i \
  ...
  LC_ALL=C \
  NODE_OPTIONS= \
  node --max-old-space-size=64 "$SCRIPT_DIR/symlink-containment-probe.mjs" \
