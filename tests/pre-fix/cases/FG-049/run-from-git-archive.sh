...
TMP=$(mktemp -d "${TMPDIR:-/tmp}/privileged-ci-ref-poc.XXXXXX")
TMP=$(CDPATH= cd -- "$TMP" && pwd -P)
WRAPPER_SENTINEL=$(mktemp "$TMP/.fg049-wrapper-owner.XXXXXX")
WRAPPER_TOKEN=${WRAPPER_SENTINEL##*.}
printf 'FG049-WRAPPER:%s\n' "$WRAPPER_TOKEN" > "$WRAPPER_SENTINEL"
WRAPPER_SENTINEL_ID=$(file_identity "$WRAPPER_SENTINEL")

cleanup() {
  status_code=$?
  trap - EXIT HUP INT TERM
  expected_owner="FG049-WRAPPER:$WRAPPER_TOKEN"
  actual_owner=
  actual_sentinel_id=
  ...
  case "$WRAPPER_SENTINEL" in
    "$TMP"/.fg049-wrapper-owner.*) sentinel_path_owned=1 ;;
...
GUARD_ROOT=$(mktemp -d "$TMP/fg049-guard.XXXXXX")
GUARD_ROOT=$(CDPATH= cd -- "$GUARD_ROOT" && pwd -P)
GUARD_SENTINEL=$(mktemp "$GUARD_ROOT/.fg049-owner.XXXXXX")
GUARD_TOKEN=${GUARD_SENTINEL##*.}
printf 'FG049-OWNER:%s\n' "$GUARD_TOKEN" > "$GUARD_SENTINEL"
...
if FG049_WRAPPER_TEMP_ROOT="$GUARD_ROOT" \
  FG049_OWNERSHIP_SENTINEL="$GUARD_SENTINEL" \
  FG049_OWNERSHIP_TOKEN="wrong-$GUARD_TOKEN" \
  node "$SCRIPT_DIR/privileged-ref-trust-probe.mjs" \
...
RUN_ROOT=$(mktemp -d "$TMP/fg049-run.XXXXXX")
RUN_ROOT=$(CDPATH= cd -- "$RUN_ROOT" && pwd -P)
RUN_SENTINEL=$(mktemp "$RUN_ROOT/.fg049-owner.XXXXXX")
...
FG049_WRAPPER_TEMP_ROOT="$RUN_ROOT" \
  FG049_OWNERSHIP_SENTINEL="$RUN_SENTINEL" \
  FG049_OWNERSHIP_TOKEN="$RUN_TOKEN" \
  node "$SCRIPT_DIR/privileged-ref-trust-probe.mjs" \
    "$RUN_ROOT/source"
