...
umask 077
TMP=$(mktemp -d "$TMP_BASE/cloudflare-access-policy-poc.XXXXXX")
TMP=$(CDPATH= cd -- "$TMP" && pwd -P)
case "$TMP" in
  "$TMP_BASE"/cloudflare-access-policy-poc.*) ;;
  *)
...
OWNER_FILE="$TMP/.cloudflare-access-policy-poc-owner"
...
printf '%s\n' '[+] safety synthetic provider JSON only; no provider, network, token, Docker, SSH, or live access'
CF_ACCESS_POC_TMP_ROOT="$TMP" \
CF_ACCESS_POC_OWNER_TOKEN="$OWNER_TOKEN" \
node --max-old-space-size=64 "$SCRIPT_DIR/cloudflare-access-policy-exactness-probe.mjs" "$SOURCE_REAL"
