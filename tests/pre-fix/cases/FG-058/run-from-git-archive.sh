...
TMP=$(mktemp -d "$TMP_PARENT/remote-evidence-revision-poc.XXXXXX")
TMP=$(CDPATH= cd -- "$TMP" && pwd -P)
TMP_ID=$(file_identity "$TMP")
WRAPPER_SENTINEL=$(mktemp "$TMP/.fg058-wrapper-owner.XXXXXX")
WRAPPER_TOKEN=${WRAPPER_SENTINEL##*.}
printf 'FG058-WRAPPER:%s\n' "$WRAPPER_TOKEN" > "$WRAPPER_SENTINEL"
WRAPPER_SENTINEL_ID=$(file_identity "$WRAPPER_SENTINEL")

cleanup() {
  status_code=$?
  trap - EXIT HUP INT TERM
  expected_owner="FG058-WRAPPER:$WRAPPER_TOKEN"
  actual_owner=
  actual_sentinel_id=
  ...
  case "$WRAPPER_SENTINEL" in
    "$TMP"/.fg058-wrapper-owner.*) sentinel_path_owned=1 ;;
...
GUARD_ROOT=$(mktemp -d "$TMP/fg058-guard.XXXXXX")
GUARD_ROOT=$(CDPATH= cd -- "$GUARD_ROOT" && pwd -P)
GUARD_SENTINEL=$(mktemp "$GUARD_ROOT/.fg058-owner.XXXXXX")
GUARD_TOKEN=${GUARD_SENTINEL##*.}
printf 'FG058-OWNER:%s\n' "$GUARD_TOKEN" > "$GUARD_SENTINEL"
...
if env -i PATH="$PATH" HOME="$GUARD_ROOT" LANG=C LC_ALL=C NODE_OPTIONS= \
  FG058_WRAPPER_TEMP_ROOT="$GUARD_ROOT" \
  FG058_OWNERSHIP_SENTINEL="$GUARD_SENTINEL" \
  FG058_OWNERSHIP_TOKEN="wrong-$GUARD_TOKEN" \
  node "$SCRIPT_DIR/remote-revision-binding-probe.mjs" \
...
RUN_ROOT=$(mktemp -d "$TMP/fg058-run.XXXXXX")
RUN_ROOT=$(CDPATH= cd -- "$RUN_ROOT" && pwd -P)
RUN_SENTINEL=$(mktemp "$RUN_ROOT/.fg058-owner.XXXXXX")
...
env -i PATH="$PATH" HOME="$RUN_ROOT" LANG=C LC_ALL=C NODE_OPTIONS= \
  FG058_WRAPPER_TEMP_ROOT="$RUN_ROOT" \
  FG058_OWNERSHIP_SENTINEL="$RUN_SENTINEL" \
  FG058_OWNERSHIP_TOKEN="$RUN_TOKEN" \
  node "$SCRIPT_DIR/remote-revision-binding-probe.mjs" \
