...
source_fingerprint() {
  {
    sha256_file "$1/scripts/hosted-workload-contract.mjs"
    sha256_file "$1/scripts/compose-vps.sh"
    sha256_file "$1/scripts/prepare-hosted-workloads.sh"
    sha256_file "$1/scripts/hosted-workload-contract.test.mjs"
...
TMP=$(mktemp -d "$TMP_PARENT/fg073-hosted-config-poc.XXXXXX")
...
cleanup() {
  ...
  expected_sentinel="$TMP/.fg073-wrapper-owner-$OWNER_TOKEN"
  physical_tmp=
  actual_parent_id=
  actual_tmp_id=
  actual_sentinel_id=
  ...
  case "$TMP" in
    "$TMP_PARENT"/fg073-hosted-config-poc.*) temp_name_valid=true ;;
  ...
  if [ "$temp_name_valid" = true ] \
    ...
    && [ "$(cat "$OWNER_SENTINEL")" = "fg073-hosted-config:$OWNER_TOKEN" ]; then
    if rm -rf -- "$TMP"; then
...
OWNER_SENTINEL="$TMP/.fg073-wrapper-owner-$OWNER_TOKEN"
[ ! -e "$OWNER_SENTINEL" ] || {
  printf '%s\n' 'ownership sentinel target unexpectedly exists' >&2
  exit 1
}
printf 'fg073-hosted-config:%s\n' "$OWNER_TOKEN" > "$OWNER_SENTINEL"
...
safe_git -C "$SOURCE_REPO" archive --format=tar --output "$SOURCE_ARCHIVE" "$REVISION"
...
if env -i PATH="$PATH" HOME="$TMP/node-home" LANG=C LC_ALL=C NODE_OPTIONS= \
  FG073_ARCHIVE_SHA256="$ARCHIVE_SHA256" node "$SCRIPT_DIR/hosted-config-provenance-probe.mjs" guard \
...
printf '%s\n' '[+] safety exact archived source and synthetic in-memory values only; no core env, secret, Docker, Compose, deploy, mount, provider, or network operation'
env -i PATH="$PATH" HOME="$TMP/node-home" LANG=C LC_ALL=C NODE_OPTIONS= \
  FG073_ARCHIVE_SHA256="$ARCHIVE_SHA256" node "$SCRIPT_DIR/hosted-config-provenance-probe.mjs" run \
