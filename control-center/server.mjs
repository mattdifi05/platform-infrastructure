...
const server = createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/__health") {
      ...
      json(res, { ok: true, service: "control-center", firstConfiguration: setup.state });
      return;
    }
    ...
    if (firstConfiguration.enabled) {
      ...
      if (!setup.complete) {
        ...
        const controlApi = url.pathname === "/control" || url.pathname.startsWith("/control/");
        if (["GET", "HEAD"].includes(method) && !controlApi) {
          redirect(res, "/first-configuration");
        } else {
          json(res, {
            error: "first_configuration_incomplete",
            message: "Complete the guided first configuration before using Control Center operations.",
          }, 423);
        }
        return;
      }
