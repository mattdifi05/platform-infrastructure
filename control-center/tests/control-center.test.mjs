...
test("Control Center production startup fails closed without app-passkey configuration", async () => {
  const port = await freePort();
  const child = spawn(process.execPath, [path.join(infraRoot, "control-center", "server.mjs")], {
    cwd: infraRoot,
    env: {
  ...
  assert.match(stderr, /CONTROL_CENTER_AUTH_MODE must be app-passkey/);
});
