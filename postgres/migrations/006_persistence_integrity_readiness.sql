...
ALTER TABLE platform_ops.schema_migrations
  ADD COLUMN IF NOT EXISTS applied_by text NOT NULL DEFAULT current_user,
  ADD COLUMN IF NOT EXISTS metadata jsonb NOT NULL DEFAULT '{}'::jsonb;

CREATE TABLE IF NOT EXISTS platform_ops.data_retention_policies (
...
ALTER TABLE platform_ops.backup_restore_runs
...
DROP TRIGGER IF EXISTS trg_data_retention_policies_updated_at ON platform_ops.data_retention_policies;
CREATE TRIGGER trg_data_retention_policies_updated_at
BEFORE UPDATE ON platform_ops.data_retention_policies
...
ALTER TABLE app_account.accounts
  ...
  ADD CONSTRAINT accounts_deleted_anonymized CHECK (
    deleted_at IS NULL
    OR email::text ~ '^deleted\+[0-9a-f-]+@deleted\.platform\.local$'
  );

ALTER TABLE app_account.account_security_settings
...
ALTER TABLE app_account.passkeys
...
ALTER TABLE app_account.email_otp_challenges
...
ALTER TABLE app_account.backup_code_sets
...
ALTER TABLE app_account.backup_codes
...
ALTER TABLE app_account.device_approval_requests
...
ALTER TABLE app_account.subscriptions
...
ALTER TABLE app_account.email_outbox
...
WITH ranked_sets AS (
  ...
  FROM app_account.backup_code_sets
  WHERE revoked_at IS NULL
)
UPDATE app_account.backup_code_sets code_set
SET revoked_at = now()
...
DROP TRIGGER IF EXISTS trg_audit_events_append_only ON app_account.audit_events;
CREATE TRIGGER trg_audit_events_append_only
BEFORE UPDATE OR DELETE ON app_account.audit_events
FOR EACH ROW EXECUTE FUNCTION app_account.prevent_audit_event_mutation();

DROP POLICY IF EXISTS audit_events_runtime_access ON app_account.audit_events;
DROP POLICY IF EXISTS audit_events_runtime_select ON app_account.audit_events;
DROP POLICY IF EXISTS audit_events_runtime_insert ON app_account.audit_events;
CREATE POLICY audit_events_runtime_select ON app_account.audit_events
  FOR SELECT TO PUBLIC
  USING (pg_has_role(current_user, 'app_db_audit_rw', 'member'));
CREATE POLICY audit_events_runtime_insert ON app_account.audit_events
  FOR INSERT TO PUBLIC
  WITH CHECK (pg_has_role(current_user, 'app_db_audit_rw', 'member'));

REVOKE UPDATE, DELETE ON app_account.audit_events FROM app_db_audit_rw;
REVOKE UPDATE, DELETE ON app_account.audit_events FROM app_user;
GRANT SELECT, INSERT ON app_account.audit_events TO app_db_audit_rw;
ALTER DEFAULT PRIVILEGES IN SCHEMA app_account REVOKE UPDATE, DELETE ON TABLES FROM app_db_audit_rw;
...
VALUES
  ('audit_events', 'app_account.audit_events'::regclass, interval '400 days', 'archive', true, 'Security audit is retained online for investigation and archived before purge.'),
  ('email_otp_challenges', 'app_account.email_otp_challenges'::regclass, interval '30 days', 'purge', true, 'Expired OTP challenges are short-lived operational security data.'),
  ('email_outbox', 'app_account.email_outbox'::regclass, interval '90 days', 'archive', true, 'Email delivery trace is retained for support and abuse review.'),
  ('revoked_sessions', 'app_account.sessions'::regclass, interval '180 days', 'archive', true, 'Revoked and expired sessions are retained for security review.'),
  ('deleted_accounts', 'app_account.accounts'::regclass, interval '30 days', 'anonymize', true, 'Soft-deleted accounts are anonymized before any irreversible purge.')
...
VALUES
  (
    'data_retention',
    '{"onlinePolicies":["audit_events","email_otp_challenges","email_outbox","revoked_sessions","deleted_accounts"],"policyTable":"platform_ops.data_retention_policies","requiresRestoreTestBeforePurge":true}'::jsonb,
  ...
  (
    ...
    '{"backupFormat":"pg_dump custom","checksum":"sha256","restoreTest":"required","runLog":"platform_ops.backup_restore_runs"}'::jsonb,
...
SET value = jsonb_set(
      jsonb_set(value, '{retentionPoliciesTable}', '"platform_ops.data_retention_policies"'::jsonb, true),
      '{restoreRunLog}',
      '"platform_ops.backup_restore_runs"'::jsonb,
      true
    ),
    updated_at = now()
WHERE key = 'backup';

COMMENT ON TABLE platform_ops.data_retention_policies IS 'Authoritative database retention policy table for operational data classes.';
COMMENT ON TABLE platform_ops.backup_restore_runs IS 'Append-style log of PostgreSQL backup, restore and restore-test executions.';
COMMENT ON TRIGGER trg_audit_events_append_only ON app_account.audit_events IS 'Audit events are append-only; corrections must be represented by a new event.';
